...
These programs provide configurable and restartable stages; the distinction is not flexible versus rigid software. Most organize analyses around a core pipeline. PhyloProcessR instead exposes phylogenomics-specific transformations and assessments as independent R functions, with workflows as examples. Its scope crosses assembly, variant processing, alignment curation, and data integration, so contributing one feature to a stage-specific pipeline would not provide equivalent function-level composition. The functions can also augment outputs from other pipelines.

The assembly strategy also differs from HybPiper in sequence and purpose. HybPiper first assigns reads to reference targets, assembles each target separately, and extracts coding sequences and flanking introns. PhyloProcessR first performs a whole-library de novo assembly, then optionally uses sensitive matching to identify divergent target contigs that the initial target filter missed. `assembleBinnedTargets()` uses these sample-derived contigs, rather than only the reference targets, to recruit reads for additive per-locus reassembly; longer supported contigs can replace shorter results, while unresolved multicopy targets remain unchanged by default. This design can recover weak or divergent targets and extend flanks, including for non-coding markers, but this draft reports no head-to-head benchmark. It is therefore a testable design distinction, not a claim of superior recovery.

More broadly, PhyloProcessR lets researchers retain preferred assemblers, mappers, aligners, or variant callers while using common functions for consensus processing, alignment curation, target-to-gene organization, capture assessment, paralog analysis, legacy-data integration, and shared-locus discovery. Its contribution is an R-centered combination of ready-to-use workflows and composable phylogenomic operations. Evaluation should consider correct sequence recovery, data retention, reproducibility, and the consequences of curation choices rather than function count alone.

# Software design

PhyloProcessR provides two complementary interfaces: staged reference workflows and an independently callable R function set. The repository includes workflow and configuration scripts for preprocessing, assembly, variant calling, alignment, trimming and dataset construction, joint genotyping, capture assessment, legacy integration, novel-locus discovery, and paralog analysis. These scripts demonstrate complete analyses, but the underlying functions can also be assembled directly in project-specific R scripts. Users may begin at an intermediate data product, omit unneeded stages, substitute compatible methods, and reorder operations where their input-output dependencies permit.

The current package exports 113 user-callable R functions. These functions connect domain-specific processing with established external programs: R manages files, parameters, sequence objects, summaries, and transformations, whereas external programs perform read cleaning, mapping, assembly, similarity searching, alignment, and variant calling. Distinctive operations include `extractGenomeTarget()`, which recovers target loci from matching GenBank genome assemblies; `addLegacyAlignments()`, which matches and merges Sanger or GenBank alignments with capture loci; and `discoverSharedRegions()`, `assembleSharedRegions()`, and `collectNovelContigs()`, which identify and assemble shared candidate loci from reads not assigned to known targets. Alignment curation includes `superTrimmer()`, reading-frame-aware `trimExonORF()`, native coverage, similarity, segment, and profile-HMM filters, and an interface to TrimAl. Standard FASTA, PHYLIP, and intermediate files provide boundaries for composing these operations with alternative tools.
...
PhyloProcessR, including its scientific conception, workflow design, and source code, was authored by CRH. OpenAI ChatGPT and Codex were used for debugging, test scaffolding, document formatting, software documentation, and manuscript editing. Model versions varied during development and were not consistently recorded. CRH reviewed and edited all AI-assisted outputs, made the scientific and design decisions, and verified changes with automated tests, `R CMD check`, targeted software runs, and manual review as appropriate. CRH retains responsibility for the scientific content, software behavior, licensing, and design decisions.
