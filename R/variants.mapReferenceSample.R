...
mapReferenceSample = function(mapping.directory = NULL,
  ...
  if (is.null(assembly.directory) || !dir.exists(assembly.directory)) {
    stop("Assembly directory not found.")
  }

  #Matches prepared samples to their assemblies by exact sample ID
  samples = list.dirs(mapping.directory, recursive = FALSE, full.names = FALSE)
  samples = samples[nzchar(samples)]
  assemblies = list.files(assembly.directory, pattern = "\\.(fa|fasta)$",
                          ignore.case = TRUE, full.names = TRUE)
  ids = sub("\\.(fa|fasta)$", "", basename(assemblies), ignore.case = TRUE)
  if (anyDuplicated(ids)) {
    stop("Duplicate assembly sample IDs: ",
         paste(unique(ids[duplicated(ids)]), collapse = ", "))
  }

  missing = setdiff(samples, ids)
  if (check.assemblies == TRUE && length(missing) > 0) {
    stop("Missing assemblies for sample(s): ", paste(missing, collapse = ", "))
  }
  retained = intersect(samples, ids)
  if (length(retained) == 0) { stop("No prepared samples have matching assemblies.") }
  if (check.assemblies == FALSE && length(missing) > 0) {
    message("Skipping samples without assemblies: ", paste(missing, collapse = ", "))
  }

  #Two GATK JVMs run at the same time in the mapping pipe, so the heap budget is
  #split between them.
  ...
  for (sample in retained) {
    ...
    index.dir = file.path(sample.dir, "index")
    reference = file.path(index.dir, "reference.fa")
    index.outputs = c(reference, paste0(reference, ".fai"),
                      file.path(index.dir, "reference.dict"),
                      paste0(reference, c(".amb", ".ann", ".bwt", ".pac", ".sa")))

    #################################################
    ### Part A: build the reference index for the sample
    #################################################
    #The whole index is rebuilt on overwrite or when any required file is missing
    if (overwrite == TRUE || !.stageComplete(index.dir, "reference-index", index.outputs)) {
      ...
      if (!file.copy(source, reference, overwrite = TRUE)) {
        stop("Could not copy assembly for ", sample)
      }
      log = file.path("logs/sample_logs", sample, "reference-index.stderr.log")
      .ensureDirectory(dirname(log))
      .runCommand(paste(bwa, "index -a bwtsw", shQuote(reference)),
                  quiet, "BWA reference indexing", stderr.log = log)
      .runCommand(paste(samtools, "faidx", shQuote(reference)),
                  quiet, "samtools reference indexing", stderr.log = log)
      .runCommand(paste(gatk.command, "CreateSequenceDictionary --REFERENCE", shQuote(reference),
                        "--OUTPUT", shQuote(file.path(index.dir, "reference.dict")),
                        "--USE_JDK_DEFLATER true --USE_JDK_INFLATER true"),
                  quiet, "reference dictionary", stderr.log = log)
      if (!all(file.exists(index.outputs))) {
        stop("Reference indexing did not produce every required file for ", sample)
      }
      .markStageComplete(index.dir, "reference-index", paste0("assembly=", normalizePath(source)))
    }

    #################################################
    ### Part B: map each prepared lane to the reference
    #################################################
    ...
    for (j in seq_along(lanes)) {
      ...
      if (overwrite == FALSE && .stageComplete(lane.dir, "mapReferenceSample", c(output, index))) {
        next
      }
      .invalidateStage(lane.dir, "mapReferenceSample")
      ...
      pipeline = paste(gatk.command, "SamToFastq -I", shQuote(input.bams[j]),
                       ...
                       "-USE_JDK_DEFLATER true -USE_JDK_INFLATER true |",
                       bwa, "mem -M -p -t", threads, shQuote(reference), "/dev/stdin |",
                       gatk.command, "MergeBamAlignment -ALIGNED_BAM /dev/stdin -UNMAPPED_BAM",
                       shQuote(input.bams[j]), "-OUTPUT", shQuote(cleaned), "-R", shQuote(reference),
