...
prepareBAM = function(read.directory = NULL,
  ...
  #Quick checks
  if (is.null(read.directory) || !dir.exists(read.directory)) {
    stop("Input read directory not found.")
  }
  samples = .listSampleNames(read.directory)
  if (length(samples) == 0) { stop("No sample directories or FASTQ files were found.") }
  ...
  #Prepares each sample in parallel
  results = parallel::mclapply(samples, function(sample) {
    tryCatch({
      #Reads live either in a per-sample sub-directory or flat in read.directory
      ...
      if (length(reads) == 0) { stop("no FASTQ files found") }

      #Groups reads into lanes by the stripped read prefix
      ...
      if (length(prefixes) == 0) { stop("no recognizable read pairs found") }
      ...
      for (j in seq_along(prefixes)) {
        ...
        ordered = tryCatch(.orderReadFiles(lane.reads, allow.third = TRUE),
                           error = function(e) {
        ...
        if (overwrite == FALSE && .stageComplete(lane.dir, "prepareBAM", output)) { next }
        .invalidateStage(lane.dir, "prepareBAM")
        unlink(output)
        ...
        #Builds the read-group ID from the first FASTQ header, or a placeholder
        if (auto.readgroup == TRUE) {
          con = if (grepl("\\.gz$", pair[1], ignore.case = TRUE)) {
            gzfile(pair[1], "rt")
          } else {
            file(pair[1], "rt")
          }
          ...
          fields = if (length(header)) {
            strsplit(sub("^@", "", header), ":", fixed = TRUE)[[1]]
          } else {
            character()
          }
          if (length(header) != 1 || !startsWith(header, "@") ||
              length(fields) < 4 || any(!nzchar(fields[1:4]))) {
            stop("lane ", basename(prefixes[j]),
                 " has no supported Illumina header; use auto.readgroup = FALSE")
          }
        ...
        #Converts the FASTQ pair to an unmapped BAM with read-group tags
        fastq.sam = file.path(lane.dir, "fastqsam.bam")
        reverted = file.path(lane.dir, "revertsam.bam")
        commands = c(
          paste(gatk.command, "FastqToSam -FASTQ", shQuote(pair[1]), "-FASTQ2", shQuote(pair[2]),
                "-OUTPUT", shQuote(fastq.sam), "-SAMPLE_NAME", shQuote(sample),
                "-USE_JDK_DEFLATER true -USE_JDK_INFLATER true"),
          paste(gatk.command, "RevertSam -I", shQuote(fastq.sam), "-O", shQuote(reverted),
                ...
                "-REMOVE_ALIGNMENT_INFORMATION true -USE_JDK_DEFLATER true -USE_JDK_INFLATER true"),
          paste(gatk.command, "AddOrReplaceReadGroups -I", shQuote(reverted), "-O", shQuote(output),
                "-RGSM", shQuote(sample), "-RGPU", shQuote(RGPU), "-RGID", shQuote(RGID),
                "-RGLB", shQuote(paste0("LIB-", sample)),
                "-RGPL ILLUMINA -USE_JDK_DEFLATER true -USE_JDK_INFLATER true"))
        for (k in seq_along(commands)) {
          .runCommand(commands[k], quiet, paste0("prepareBAM command ", k), stderr.log = log)
        }
        if (!file.exists(output) || file.info(output)$size == 0) {
          stop("GATK did not create ", output)
        }
