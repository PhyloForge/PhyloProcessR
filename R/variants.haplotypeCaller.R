...
#'   it is used for every sample; otherwise reference.type resolves the reference.
...
haplotypeCaller = function(mapping.directory = NULL,
  ...
  if (length(sample.names) == 0) { stop("No samples are available to analyze.") }
  ...
  #Calls each sample in parallel
  results = parallel::mclapply(sample.names, function(sample) {
    tryCatch({
      out.dir = file.path(output.directory, sample)
      .ensureDirectory(out.dir)
      gvcf = file.path(out.dir, "gatk4-haplotype-caller.g.vcf.gz")
      idx = paste0(gvcf, ".tbi")

      #Skips a completed sample, otherwise clears the stage before recomputing
      if (overwrite == FALSE && .stageComplete(out.dir, "haplotypeCaller", c(gvcf, idx))) {
        return(list(success = TRUE))
      }
      .invalidateStage(out.dir, "haplotypeCaller")
      unlink(c(gvcf, idx))

      reference = if (!is.null(reference.path)) {
        reference.path
      } else if (reference.type == "sample") {
        file.path(mapping.directory, sample, "index", "reference.fa")
      } else {
        file.path("index", "reference.fa")
      }
      if (!file.exists(reference)) { stop("Reference not found: ", reference) }

      #Uses the original lane BAMs only. Lane_Merge is an output, not an input.
      ...
      if (length(bams) == 0) { stop("No completed source lane BAMs found") }

      log = file.path("logs/sample_logs", sample, "haplotypeCaller.stderr.log")
      .ensureDirectory(dirname(log))

      #Merges, sorts, marks duplicates, and finalizes when there is more than one
      #lane. A single lane is called directly.
      if (length(bams) > 1) {
        merge.dir = file.path(mapping.directory, sample, "Lane_Merge")
        .ensureDirectory(merge.dir)
        merged = file.path(merge.dir, "final-mapped-merge.bam")
        sorted = file.path(merge.dir, "final-mapped-sort.bam")
        dup = file.path(merge.dir, "final-mapped-dup.bam")
        input.bam = file.path(merge.dir, "final-mapped-all.bam")

        inputs = paste(rep("-I", length(bams)), shQuote(bams), collapse = " ")
        .runCommand(paste(command, "MergeSamFiles", inputs, "-O", shQuote(merged),
                          "-USE_JDK_DEFLATER true -USE_JDK_INFLATER true"),
                    quiet, "lane merge", stderr.log = log)
        .runCommand(paste(command, "SortSam -INPUT", shQuote(merged), "-OUTPUT", shQuote(sorted),
                          "-CREATE_INDEX true -SORT_ORDER coordinate",
                          "-USE_JDK_DEFLATER true -USE_JDK_INFLATER true"),
                    quiet, "merged BAM sort", stderr.log = log)
        .runCommand(paste(command, "MarkDuplicates -INPUT", shQuote(sorted), "-OUTPUT", shQuote(dup),
                          "-CREATE_INDEX true -METRICS_FILE",
                          shQuote(file.path("logs/sample_logs", sample, "duplicate_metrics.txt")),
                          "-USE_JDK_DEFLATER true -USE_JDK_INFLATER true"),
                    quiet, "merged duplicate marking", stderr.log = log)
        .runPipeline(paste(command, "SortSam -INPUT", shQuote(dup),
                           "-OUTPUT /dev/stdout -SORT_ORDER coordinate",
                           "-USE_JDK_DEFLATER true -USE_JDK_INFLATER true |",
                           command, "SetNmAndUqTags -INPUT /dev/stdin -OUTPUT", shQuote(input.bam),
                           "-CREATE_INDEX true -R", shQuote(reference),
                           "-USE_JDK_DEFLATER true -USE_JDK_INFLATER true"),
                     quiet, "merged BAM finalization", log)
        if (!file.exists(input.bam)) { stop("Merged BAM was not created") }
        # GATK -CREATE_INDEX writes <base>.bai, replacing the .bam extension.
        file.remove(c(merged, sorted, dup, sub("\\.bam$", ".bai", c(sorted, dup))))
      } else {
        input.bam = bams[1]
      }

      .runCommand(paste(command, "HaplotypeCaller -R", shQuote(reference), "-O", shQuote(gvcf),
                        "-I", shQuote(input.bam), "-ERC GVCF -ploidy", ploidy,
                        "--native-pair-hmm-threads 1 -bamout",
                        shQuote(file.path(out.dir, "gatk4-haplotype-caller.bam"))),
                  quiet, "HaplotypeCaller", stderr.log = log)
      if (!all(file.exists(c(gvcf, idx)))) {
        stop("HaplotypeCaller did not create the GVCF and index")
      }
      .markStageComplete(out.dir, "haplotypeCaller",
                         c(paste0("bam=", input.bam), paste0("ploidy=", ploidy)))
      list(success = TRUE)
