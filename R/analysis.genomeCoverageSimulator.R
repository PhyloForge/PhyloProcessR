...
genomeCoverage = function(genome.directory = NULL,
  ...
  for (i in 1:length(number.gb)){
    ...
    system(paste0(samtools.path, "mosdepth --no-per-base --fast-mode --use-median ",
                  ...
                  " ", output.directory, "/", sample.name, " ", output.directory, "/mapped-reads.bam"))

    #Reads in depth information
    depth.summary = data.table::fread(paste0(output.directory, "/", sample.name, ".mosdepth.summary.txt"), header = T)
    ...
    region.summary = data.table::fread(paste0(output.directory, "/", sample.name, ".mosdepth.region.dist.txt"), header = F)
    ...
    data.table::set(collect.data, i = as.integer(i), j = match("no_coverage", header.data), value = (total.depth$x[1]-sum(total.depth$x[2:nrow(total.depth)]))*100 )
    data.table::set(collect.data, i = as.integer(i), j = match("above_1x", header.data), value = sum(total.depth$x[2:nrow(total.depth)])*100 )
    data.table::set(collect.data, i = as.integer(i), j = match("above_2x", header.data), value = sum(total.depth$x[3:nrow(total.depth)])*100 )
    data.table::set(collect.data, i = as.integer(i), j = match("above_3x", header.data), value = sum(total.depth$x[4:nrow(total.depth)])*100 )
    data.table::set(collect.data, i = as.integer(i), j = match("above_4x", header.data), value = sum(total.depth$x[5:nrow(total.depth)])*100 )
    data.table::set(collect.data, i = as.integer(i), j = match("above_5x", header.data), value = sum(total.depth$x[6:nrow(total.depth)])*100 )
    data.table::set(collect.data, i = as.integer(i), j = match("above_10x", header.data), value = sum(total.depth$x[11:nrow(total.depth)])*100 )
    data.table::set(collect.data, i = as.integer(i), j = match("above_20x", header.data), value = sum(total.depth$x[21:nrow(total.depth)])*100 )
